...
| 配置块 | 面板内由「⚙ 配置」按钮展开的编辑区：项目与启动配置的增删改。收起时零占位 | 设置页里的表单（曾短暂如此，已改回面板内） |
...
- **视图占满面板**：本视图在场时收起底部的消息输入框（`[data-composer-seat]`），与 上下文 tab 的行为一致。DSH 没有按视图隐藏输入框的 API（`conversation.composer` 链的 `select` 只能拿到 `sessionId`/`session`/`pendingInteraction`），因此与 `dsh-context` 同法用 CSS `:has(.ide-view)` 按视图根元素收起，并保留承载审批/追问/计划复核时的输入框。
- **配置在面板内编辑**：头部「⚙ 配置」按钮展开编辑块（项目增删、启动配置的名称/启动命令/工作目录/环境变量），收起时零占位。**单一写者**：只有这一处写配置，不注册设置页——两个面都整份写盘会互相覆盖。
- tab 管理对齐主流 IDE：横向滑动（滚轮/拖动）、`»` 溢出菜单、`×` 关闭、只看运行中。
- 配置块：项目增删（从 DSH 工作区里挑）、每个项目下启动配置增删改（名称 / 启动命令 / 工作目录 / 环境变量），保存有 `已保存 ✓` 反馈，失败回滚到磁盘真实状态。
...
1. 会话视图 tab 排里出现第四个 tab「IDE」（对话 / 轨迹 / 上下文 / IDE），点进去是运行控制台；「⚙ 配置」可在面板内增删改项目与启动配置，收起后不占位；DSH 重启后配置与顺序仍在（持久化生效）。
...
- Client：`host.call`、`styles.insert`、`ctx.interval`、`React.createElement`；唯一注册点 `conversation.view`（id `dsh-newbe-ide`、order 30、label `IDE`）。
